    ...
    <div className="space-y-5 px-4 pb-28 pt-4">
      <section className="rounded-[2rem] border border-white/70 bg-white/90 p-5 shadow-[0_24px_80px_-54px_rgba(15,23,42,0.3)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">{order.order_number}</h1>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-[1.25rem] bg-slate-50 px-4 py-3">
            ...
            <p className="mt-2 text-lg font-semibold text-slate-950">{order.store?.name ?? "—"}</p>
          </div>
          <div className="rounded-[1.25rem] bg-slate-50 px-4 py-3">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">To'lov</p>
            <p className="mt-2 text-lg font-semibold text-slate-950">{order.payment_method}</p>
          </div>
          <div className="rounded-[1.25rem] bg-slate-50 px-4 py-3">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Jami</p>
            <p className="mt-2 text-lg font-semibold text-slate-950">{formatMoney(order.total_price)}</p>
          </div>
        </div>
      </section>
        ...
        <div className="mt-4 space-y-3">
          {order.items.map((item) => (
            ...
            >
              ...
              <p className="font-semibold text-slate-950">{formatMoney(item.total_price)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-[2rem] border border-white/70 bg-white/90 p-5 shadow-sm">
          ...
          <p className="mt-4 text-sm text-slate-600">{order.delivery_address}</p>
          {order.delivery_details ? (
            <p className="mt-2 text-sm text-slate-500">{order.delivery_details}</p>
          ) : null}
        </div>

        <div className="rounded-[2rem] border border-white/70 bg-white/90 p-5 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-950">To'lov tafsiloti</h2>
