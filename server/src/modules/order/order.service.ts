...
export class OrderService {
  ...
  async submitDeliveryCash(
    ...
    await this.walletService.freezeChangeForOrder(
      sellerUserId,
      change,
      orderId,
    );
  ...
  async getCourierLocation(orderId: string) {
    ...
    if (!order)
      throw new NotFoundException('Order not found or not in delivery');
